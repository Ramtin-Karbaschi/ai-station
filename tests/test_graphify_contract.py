...
class GraphifyContractTests(unittest.TestCase):
    ...
    def test_cli_exposes_graphify_and_defaults_to_code_only(self) -> None:
        ...
        self.assertIn("ai graphify install|configure|status|extract|query|path|explain|view|uninstall", dispatcher)
        self.assertIn("ai output show|set|open|path", dispatcher)
        self.assertIn('source "$ROOT/scripts/lib/ai-graphify.sh"', dispatcher)
        self.assertIn('source "$ROOT/scripts/lib/ai-output.sh"', dispatcher)
        ...
        self.assertIn("cmd_graphify_view", module)
        self.assertIn("--out|--output", module)
